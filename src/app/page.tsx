...
type Iteration = { id: number; request: string; summary: string; status: string; created_at: string };
type Project = { id: number; name: string; repo: string };

const seedIterations: Iteration[] = [
...
export default function Home() {
  ...
  useEffect(() => { fetch(apiUrl("/api/workspace")).then((response) => response.json()).then((data) => { setProject(data.project); if (data.iterations.length) setIterations(data.iterations); }).catch(() => setNotice("后台 API 暂时无法访问，请检查 NEXT_PUBLIC_API_BASE_URL。")); }, []);

  async function submitIteration(event: FormEvent) {
    event.preventDefault();
    if (!request.trim()) return;
    const response = await fetch(apiUrl("/api/workspace"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "iterate", text: request }) });
  ...
  async function connectRepo(event: FormEvent) {
    event.preventDefault();
    const response = await fetch(apiUrl("/api/workspace"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "connect", repo, token }) });
    const data = await response.json();
  ...
  async function publish() {
    const response = await fetch(apiUrl("/api/workspace"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "publish" }) });
  ...
  return <div className="shell">
    ...
    <div className="layout">
      <aside className="sidebar"><div className="eyebrow">Project</div><div className="project-name">{project.name}</div><nav className="nav">{["Overview", "Iterations", "Deployments"].map((item, index) => <button className={active === item ? "active" : ""} onClick={() => setActive(item)} key={item}><span className="nav-icon">{["○", "↗", "□"][index]}</span>{item}</button>)}</nav><div className="sidebar-footer">Last synced<br /><strong>just now</strong><br /><br />Built for fast-moving teams.</div></aside>
